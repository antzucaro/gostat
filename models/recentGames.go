...
func GetRecentGames(limit int, offset int) []RecentGame {
    ...
    for rows.Next() {
        rows.Scan(&gameType, &serverID, &serverName, &mapID, &mapName, 
            &gameDate, &winnerID, &winnerNick)

        rg := RecentGame{gameType, serverID, qstr.QStr(serverName), mapID,
            mapName, gameDate, winnerID, qstr.QStr(winnerNick)}
